...
static u16 GetButtonsFromKbData(const CellKbData &state)
{
	u16 buttons = 0;

	// Modifiers are stored in mkey bitmask
	if (state.mkey != 0xFFFFFFFF)
	{
		if (state.mkey & (CELL_KB_MKEY_L_SHIFT | CELL_KB_MKEY_R_SHIFT))
		{
			buttons |= TH_BUTTON_FOCUS;
		}
		if (state.mkey & (CELL_KB_MKEY_L_CTRL | CELL_KB_MKEY_R_CTRL))
		{
	...
	for (int i = 0; i < len; i++)
	{
		...
		u16 keycode = state.keycode[i] & 0xFF;
