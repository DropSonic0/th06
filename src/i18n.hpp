...
#ifndef THEN
// JP
#define TH_EN_CONFIG_FILE "東方紅魔郷.cfg"
#define TH_EN_WINDOW_TITLE "東方紅魔郷　〜 the Embodiment of Scarlet Devil"
#define TH_EN_DBG_MAINMENU_VRAM "Debug : title 開始 VRAM = %d\n"
#define TH_EN_DBG_RESULTSCREEN_COUNAT "counat = %d\n"
#define TH_EN_ERR_ALREADY_RUNNING "二つは起動できません\n"
#define TH_EN_ERR_D3D_COULD_NOT_CREATE_OBJ "Direct3D オブジェクトは何故か作成出来なかった\n"
#define TH_EN_ERR_LOGGER_START "東方動作記録 --------------------------------------------- \n"
#define TH_EN_ERR_LOGGER_END "---------------------------------------------------------- \n"
#define TH_EN_ERR_NO_PAD_FOUND "使えるパッドが存在しないようです、残念\n"
#define TH_EN_ERR_OPTION_CHANGED_RESTART "再起動を要するオプションが変更されたので再起動します\n"
#define TH_EN_ERR_SCREEN_INIT_32BITS "初回起動、画面を 32Bits で初期化しました\n"
#define TH_EN_ERR_SCREEN_INIT_16BITS "初回起動、画面を 16Bits で初期化しました\n"
#define TH_EN_ERR_SET_REFRESH_RATE_60HZ "リフレッシュレートを60Hzに変更します\n"
#define TH_EN_ERR_TL_HAL_UNAVAILABLE "T&L HAL は使用できないようです\n"
#define TH_EN_ERR_HAL_UNAVAILABLE "HAL も使用できないようです\n"
#define TH_EN_ERR_D3D_INIT_FAILED "Direct3D の初期化に失敗、これではゲームは出来ません\n"
#define TH_EN_ERR_BACKBUFFER_NONLOCKED "バックバッファをロック不可能にしてみます\n"
#define TH_EN_ERR_CANT_CHANGE_REFRESH_RATE_FORCE_VSYNC "リフレッシュレートが変更できません、vsync 非同期に変更します\n"
#define TH_EN_USING_REF_MODE "REF で動作しますが、重すぎて恐らくゲームになりません...\n"
#define TH_EN_USING_HAL_MODE "HAL で動作します\n"
#define TH_EN_USING_TL_HAL_MODE "T&L HAL で動作しま〜す\n"
...
#define TH_EN_ERR_D3DFMT_A8R8G8B8_UNSUPPORTED "D3DFMT_A8R8G8B8 をサポートしていません、減色モードで動作します\n"
#define TH_EN_ERR_CONFIG_NOT_FOUND "コンフィグデータが見つからないので初期化しました\n"
#define TH_EN_ERR_CONFIG_CORRUPTED "コンフィグデータが破壊されていたので再初期化しました\n"
#define TH_EN_ERR_NO_WAVE_FILE "wave データが無いので、midi にします\n"
#define TH_EN_ERR_NOT_A_WAV_FILE "Wav ファイルじゃない? %s\n"
#define TH_EN_ERR_NO_VERTEX_BUFFER "頂点バッファの使用を抑制します\n"
#define TH_EN_ERR_NO_FOG "フォグの使用を抑制します\n"
#define TH_EN_ERR_USE_16BIT_TEXTURES "16Bit のテクスチャの使用を強制します\n"
#define TH_EN_ERR_FORCE_BACKBUFFER_CLEAR "バックバッファの消去を強制します\n"
#define TH_EN_ERR_DONT_RENDER_ITEMS "ゲーム周りのアイテムの描画を抑制します\n"
#define TH_EN_ERR_NO_GOURAUD_SHADING "グーローシェーディングを抑制します\n"
#define TH_EN_ERR_NO_DEPTH_TESTING "デプステストを抑制します\n"
#define TH_EN_ERR_FORCE_60FPS_MODE "60フレーム強制モードにします\n"
#define TH_EN_ERR_NO_TEXTURE_COLOR_COMPOSITING "テクスチャの色合成を抑制します\n"
#define TH_EN_ERR_LAUNCH_WINDOWED "ウィンドウモードで起動します\n"
#define TH_EN_ERR_FORCE_REFERENCE_RASTERIZER "リファレンスラスタライザを強制しますn"
#define TH_EN_ERR_DO_NOT_USE_DIRECTINPUT "パッド、キーボードの入力に DirectInput を使用しません\n"
#define TH_EN_ERR_DIRECTINPUT_NOT_AVAILABLE "DirectInput が使用できません\n"
#define TH_EN_ERR_DIRECTINPUT_SETDATAFORMAT_NOT_AVAILABLE "DirectInput SetDataFormat が使用できません\n"
#define TH_EN_ERR_DIRECTINPUT_SETCOOPERATIVELEVEL_NOT_AVAILABLE "DirectInput SetCooperativeLevel が使用できません\n"
#define TH_EN_ERR_DIRECTINPUT_INITIALIZED "DirectInput は正常に初期化されました\n"
#define TH_EN_ERR_PAD_FOUND "有効なパッドを発見しました\n"
#define TH_EN_ERR_FILE_CANNOT_BE_EXPORTED "ファイルが書き出せません %s\n"
#define TH_EN_ERR_FOLDER_HAS_WRITE_PROTECT_OR_DISK_FULL                                                                \
    "フォルダが書込み禁止属性になっているか、ディスクがいっぱいいっぱいになってませんか？\n"
#define TH_EN_ERR_ASCIIMANAGER_INIT_FAILED "error : 文字の初期化に失敗しました\n"
#define TH_EN_ERR_WRONG_DATA_VERSION "error : データのバージョンが違います\n"
#define TH_EN_ERR_CANNOT_BE_LOADED "%sが読み込めないです。\n"
#define TH_EN_ERR_ANMMANAGER_SPRITE_CORRUPTED "スプライトアニメ %s が読み込めません。データが失われてるか壊れています\n"
#define TH_EN_ERR_ANMMANAGER_TEXTURE_CORRUPTED "テクスチャ %s が読み込めません。データが失われてるか壊れています\n"
#define TH_EN_ERR_ANMMANAGER_UNK_TEX_FORMAT "error : イメージがαを持っていません\n"
#define TH_EN_ERR_ECLMANAGER_ENEMY_DATA_CORRUPT "敵データの読み込みに失敗しました、データが壊れてるか失われています\n"
#define TH_EN_ERR_ENDING_END_FILE_CORRUPTED "error : エンディングファイルが読み込めない、ファイルが破壊されています\n"
#define TH_EN_ERR_MAINMENU_LOAD_SELECT_SCREEN_FAILED "セレクト画面の読み込みに失敗\n"
#define TH_EN_ERR_SOUNDPLAYER_FAILED_TO_CREATE_BGM_SOUND_BUFFER "error : ストリーミング用サウンドバッファを作成出来ませんでした\n"
#define TH_EN_ERR_SOUNDPLAYER_FAILED_TO_INITIALIZE_OBJECT "DirectSound オブジェクトの初期化が失敗したよ\n"
#define TH_EN_ERR_SOUNDPLAYER_FAILED_TO_LOAD_SOUND_FILE "error : Sound ファイルが読み込めない データを確認 %s\n"
#define TH_EN_DBG_SOUNDPLAYER_INIT_SUCCESS "DirectSound は正常に初期化されました\n"
#define TH_EN_ERR_GAMEMANAGER_FAILED_TO_INITIALIZE_STAGE "error : 背景データの初期化に失敗しました\n"
#define TH_EN_ERR_GAMEMANAGER_FAILED_TO_INITIALIZE_PLAYER "error : プレイヤーの初期化に失敗しました\n"
#define TH_EN_ERR_GAMEMANAGER_FAILED_TO_INITIALIZE_BULLETMANAGER "error : 敵弾の初期化に失敗しました\n"
#define TH_EN_ERR_GAMEMANAGER_FAILED_TO_INITIALIZE_ENEMYMANAGER "error : 敵の初期化に失敗しました\n"
#define TH_EN_ERR_GAMEMANAGER_FAILED_TO_INITIALIZE_ECLMANAGER "error : 敵頭脳の初期化に失敗しました\n"
#define TH_EN_ERR_GAMEMANAGER_FAILED_TO_INITIALIZE_EFFECTMANAGER "error : エフェクトの初期化に失敗しました\n"
#define TH_EN_ERR_GAMEMANAGER_FAILED_TO_INITIALIZE_GUI "error : 2D表示の初期化に失敗しました\n"
#define TH_EN_ERR_GUI_MSG_FILE_CORRUPTED "error : メッセージファイル %s が読み込めませんでした\n"
#define TH_EN_ERR_STAGE_DATA_CORRUPTED "ステージデータが見つかりません。データが壊れています\n"
#define TH_EN_ERR_MIDI_FAILED_TO_READ_FILE "error : MIDI File が読み込めない %s \n"
#define TH_EN_ERR_MIDI_NOT_LOADED "error : まだMIDIが読み込まれていないのに再生しようとしている\n"

#define TH_EN_REIMU_A_BOMB_NAME "霊符「夢想封印」"
#define TH_EN_REIMU_B_BOMB_NAME "夢符「封魔陣」"
#define TH_EN_MARISA_A_BOMB_NAME "魔符「スターダストレヴァリエ」"
#define TH_EN_MARISA_B_BOMB_NAME "恋符「マスタースパーク」"

#define TH_EN_HAKUREI_REIMU_SPIRIT "博麗 霊夢 (霊)"
#define TH_EN_HAKUREI_REIMU_DREAM "博麗 霊夢 (夢)"
#define TH_EN_KIRISAME_MARISA_DEVIL "霧雨 魔理沙 (魔)"
#define TH_EN_KIRISAME_MARISA_LOVE "霧雨 魔理沙 (恋)"
#define TH_EN_SATSUKI_RIN_FLOWER "冴月 麟 (花)"
#define TH_EN_SATSUKI_RIN_WIND "冴月 麟 (風)"
